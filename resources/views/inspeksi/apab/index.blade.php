...
<body id="main-body" class="bg-white text-gray-800">
    ...
    <div class="max-w-6xl mx-auto px-4 py-10">
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
            @foreach($apabs->unique('id_apab') as $apab)
                <a href="{{ route('apab.hasil', $apab->id_apab) }}"
...
<div id="modal" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center hidden z-50">
    <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-md relative">
        <h2 class="text-xl font-bold mb-4">Tambah APAB Baru</h2>
        <form action="{{ route('apab.store') }}" method="POST">
            @csrf
            <div class="mb-4">
            ...
            <div class="mb-4">
                <label class="block text-gray-700 font-semibold mb-1">Isi Apab</label>
                <input type="text" name="isi_apab" required class="w-full px-3 py-2 border rounded">
            </div>
            <div class="mb-4">
