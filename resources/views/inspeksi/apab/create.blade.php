...
<head>
    ...
    <title>Dashboard Peralatan Tanggap Darurat UP2WI - APAB</title>
...
<body id="main-body" class="bg-white text-gray-800">
    ...
    <header id="main-header" class="fixed top-0 left-0 right-0 z-50 bg-primary border-b shadow px-4 py-3">
        ...
        <div class="flex flex-col text-center md:hidden">
            ...
            MONITORING ALAT PEMADAM API BERAT
            </h1>
    ...
    <div class="fixed max-w-6xl mx-auto px-4 mt-6">
        <a href="{{ route('apab.hasil', ['id_apab' => $apab->id_apab]) }}"
    ...
    <div class="px-5 sm:px-6 md:px-8 lg:px-10">
        ...
        class="max-w-3xl mx-auto py-6 mt-8 mb-10 bg-gray-100 rounded-xl shadow-lg space-y-6 px-6 sm:px-8">    
            ...
            <input type="hidden" name="id_apab" value="{{ $apab->id_apab }}">
            <input type="hidden" name="lokasi" value="{{ $apab->lokasi }}">
            <input type="hidden" name="isi_apab" value="{{ $apab->isi_apab }}">
            <input type="hidden" name="kapasitas" value="{{ $apab->kapasitas }}">
            <input type="hidden" name="masa_berlaku" value="{{ $apab->masa_berlaku }}">
            <input type="hidden" name="catatan" value="{{ $apab->catatan }}">            
...
document.getElementById('submit-btn').addEventListener('click', function () {
  ...
  const idApab = document.querySelector('input[name="id_apab"]').value;
  ...
  .then(() => {
    ...
    setTimeout(() => {
      window.location.href = `/apab/${idApab}/hasil`;
    }, 1600);
  })
  .catch(error => {
    // 4. Kalau gagal
