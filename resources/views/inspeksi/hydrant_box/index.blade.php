...
<body class="bg-white text-gray-800" id="main-body">
    ...
    <div class="max-w-6xl mx-auto px-4 py-10">
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
            @php
            $sorted = $boxhydrants->unique('id_boxhydrant')->sortBy('id_boxhydrant', SORT_NATURAL);
            @endphp
    ...
    <div id="modal" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center hidden z-50">
        <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-md relative">
            <h2 class="text-xl font-bold mb-4">Tambah BOX HYDRANT Baru</h2>
            <form action="{{ route('boxhydrant.store') }}" method="POST">
                @csrf
                <div class="mb-3">
                    <label class="block font-semibold">ID Box Hydrant</label>
                    <input type="text" name="id_boxhydrant" required class="w-full px-3 py-2 border rounded" placeholder="cth: A4.001">
                </div>
                <div class="mb-3">
