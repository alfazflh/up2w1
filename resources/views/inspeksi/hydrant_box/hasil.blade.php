...
<head>
    ...
    <title>Dashboard Peralatan Tanggap Darurat UP2WI - BOX HYDRANT</title>
...
<body id="main-body" class="bg-white text-gray-800">
    ...
    <div class="fixed max-w-6xl mx-auto px-4 mt-6">
        <a href="{{ route('boxhydrant.index', ['id_boxhydrant' => $boxhydrant->id_boxhydrant]) }}"
                    ...
                    @foreach ($pemeriksaans as $pemeriksaan)
                        @if (!empty($pemeriksaan->tanggal_pemeriksaan))
                            <tr>
                                ...
                                <td class="px-4 py-3">
                                    BOX HYDRANT {{ $pemeriksaan->id_boxhydrant }} Bulan
                                    {{ \Carbon\Carbon::parse($pemeriksaan->tanggal_pemeriksaan)->translatedFormat('F Y') }}
                                </td>
                                <td class="px-4 py-3 text-center space-x-2">
                                    @php
                                    $data = [
                                        'id_boxhydrant' => $pemeriksaan->id_boxhydrant,
                                        ...
                                        'pilar_hydrant' => $pemeriksaan->pilar_hydrant,
                                        'box_hydrant' => $pemeriksaan->box_hydrant,
                                        'noozle' => $pemeriksaan->noozle,
                                        'selang' => $pemeriksaan->selang,
                                        'uji_fungsi' => $pemeriksaan->uji_fungsi,
                                    ...
                                    <div class="flex flex-wrap justify-center gap-2">
                                        ...
                                        @auth
                                            ...
                                            <form id="delete-form-{{ $pemeriksaan->id }}"
                                                  action="{{ route('pemeriksaan-boxhydrant.destroy', $pemeriksaan->id) }}"
...
<div id="detailModal" class="fixed inset-0 z-50 hidden bg-black bg-opacity-50 items-center justify-center">
    <div class="bg-white relative" style="border-radius: 8px; width: 750px">
                ...
                <tr>
                    ...
                    <td style="width: 68%; height: 100%; border-left: none; border-right: none; border-top: 1px solid black; border-bottom: 1px solid black;">
                        <div class="title-section">
                            <h1>KARTU KENDALI PILAR HYDRANT</h1>
        ...
        <tr>
            ...
            <td style="border: 1px solid black; padding: 3px;">NO. SERI</td>
            <td style="border: 1px solid black; padding: 3px;">: <span id="detail-nomor">-</span></td>
        </tr>
        <tr>
            <td style="border: 1px solid black; padding: 3px; width: 18%;">NAMA</td>
...
function showDetail(data) {
    ...
    try {
        ...
        document.getElementById('detail-nomor').innerText = data.id_boxhydrant || '-';
    ...
    <tr>
        ...
        <td class="checkbox-group">
            <label class="option">
                <span class="checkbox ${ (data.pilar_hydrant || '').toLowerCase() === 'baik' ? 'checked baik' : '' }"></span>
                Baik
            </label>
            <label class="option">
                <span class="checkbox ${ (data.pilar_hydrant || '').toLowerCase() === 'tidak baik' ? 'checked tidak-baik' : '' }"></span>
                Tidak Baik
            </label>
        </td>
    </tr>
    <tr>
        ...
        <td class="checkbox-group">
            <label class="option">
                <span class="checkbox ${ (data.box_hydrant || '').toLowerCase() === 'baik' ? 'checked baik' : '' }"></span>
                Baik
            </label>
            <label class="option">
                <span class="checkbox ${ (data.box_hydrant || '').toLowerCase() === 'tidak baik' ? 'checked tidak-baik' : '' }"></span>
                Tidak Baik
            </label>
        </td>
    </tr>
    <tr>
        ...
        <td class="checkbox-group">
            <label class="option">
                <span class="checkbox ${ (data.noozle || '').toLowerCase() === 'baik' ? 'checked baik' : '' }"></span>
                Baik
            </label>
            <label class="option">
                <span class="checkbox ${ (data.noozle || '').toLowerCase() === 'tidak baik' ? 'checked tidak-baik' : '' }"></span>
                Tidak Baik
            </label>
        </td>
    </tr>
    ...
    <tr>
        ...
        <td class="checkbox-group">
            <label class="option">
                <span class="checkbox ${ (data.uji_fungsi || '').toLowerCase() === 'baik' ? 'checked baik' : '' }"></span>
                Baik
            </label>
            <label class="option">
                <span class="checkbox ${ (data.uji_fungsi || '').toLowerCase() === 'tidak baik' ? 'checked tidak-baik' : '' }"></span>
                Tidak Baik
            </label>
        </td>
    </tr>
    ...
    const tanggalElem = document.getElementById('detail-tanggal');
    let bulanTahun = "Pemeriksaan";

    if (tanggalElem) {
        const tanggalText = tanggalElem.innerText.trim();
        console.log("Isi detail-tanggal:", tanggalText);
        ...
        if (/^\d{2}\/\d{2}\/\d{4}$/.test(tanggalText)) {
            ...
            bulanTahun = `${bulanIndonesia[parseInt(mm) - 1]} ${yyyy}`;
        } else if (/\d{1,2}\s+[A-Za-z]+/.test(tanggalText)) {
            const parts = tanggalText.split(" ");
            bulanTahun = `${parts[1]} ${parts[2]}`;
        }
    }

    const filename = 'Kartu Kendali BOX HYDRANT ' + bulanTahun.replace(/\s+/g, ' ') + '.pdf';
    ...
    fetch(`/inspeksi/pemeriksaan-box-hydrant/${id}/edit-form`, {
