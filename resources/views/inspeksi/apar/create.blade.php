...
<head>
    ...
    <title>Dashboard Peralatan Tanggap Darurat UP2WI - APAR</title>
...
<body id="main-body" class="bg-white text-gray-800">
    ...
    <div class="fixed max-w-6xl mx-auto px-4 mt-6">
        <a href="{{ route('apar.hasil', ['id_apar' => $apar->id_apar]) }}"
    ...
    <div class="px-5 sm:px-6 md:px-8 lg:px-10">
        <form id="pemeriksaan-form" action="{{ route('pemeriksaan-apar.store') }}" method="POST"
        class="max-w-3xl mx-auto py-6 mt-8 mb-10 bg-gray-100 rounded-xl shadow-lg space-y-6 px-6 sm:px-8">    
            @csrf
    
            <input type="hidden" name="id_apar" value="{{ $apar->id_apar }}">
            <input type="hidden" name="lokasi" value="{{ $apar->lokasi }}">
            <input type="hidden" name="isi_apar" value="{{ $apar->isi_apar }}">
            <input type="hidden" name="kapasitas" value="{{ $apar->kapasitas }}">
            <input type="hidden" name="masa_berlaku" value="{{ $apar->masa_berlaku }}">
