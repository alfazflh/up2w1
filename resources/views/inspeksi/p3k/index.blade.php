...
<head>
    <meta charset="UTF-8">
    <title>Dashboard Peralatan Tanggap Darurat UP2WI - P3K</title>
...
<body class="bg-white text-gray-800" id="main-body">
    ...
    <div class="max-w-6xl mx-auto px-4 py-10">
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
            @php
            $sorted = $p3ks->unique('id_p3k')->sortBy('id_p3k', SORT_NATURAL);
            @endphp

            @foreach($sorted as $p3k)
                <a href="{{ route('p3k.show', $p3k->id_p3k) }}"
    ...
    <div id="modal" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center hidden z-50">
        <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-md relative">
            <h2 class="text-xl font-bold mb-4">Tambah P3K Baru</h2>
            <form action="{{ route('p3k.store') }}" method="POST">
                @csrf
                <div class="mb-3">
                    <label class="block font-semibold">LOKASI P3K</label>
                    <input type="text" name="id_p3k" required class="w-full px-3 py-2 border rounded">
                </div>
                @php
                $defaultCatatan = $p3ks->first()->catatan ?? '';
                @endphp
